...
const ENLACES = [
  ...
  { href: '/calendario-academico', label: 'Calendario Académico 2026' },
      ...
      <div className="relative container mx-auto max-w-6xl px-5 pt-10 pb-20 sm:px-6 lg:pt-12 lg:pb-24">
        <div className="grid grid-cols-1 text-center lg:grid-cols-[1.7fr_1fr_1.15fr_1.2fr] lg:gap-y-0 lg:text-left">
          ...
          <nav className={COLUMNA} aria-label="Enlaces del sitio">
            ...
            <ul className="grid grid-cols-2 gap-x-4 text-[15px] lg:grid-cols-1 lg:text-sm">
              {ENLACES.map(({ href, label }) => (
                <li key={href} className="border-b border-[rgba(0,199,177,0.1)] lg:last:border-b-0">
          ...
          <div className={COLUMNA}>
            ...
            <ul className="grid w-full grid-cols-2 items-start gap-x-3 text-left text-[13px] lg:block lg:w-auto lg:space-y-4 lg:text-sm">
              <li className="flex min-w-0 items-start gap-2 lg:gap-3">
                ...
                <a href={MAPS_URL} target="_blank" rel="noopener" className="min-w-0 min-h-10 py-0.5 leading-5 text-[#c0d5d0] transition-colors hover:text-white focus-visible:outline-none focus-visible:text-white lg:min-h-0 lg:py-0 lg:leading-normal">
                  Guaminí 4876, Piso 1<br />Villa Lugano, CABA
                </a>
              </li>
              <li className="flex min-w-0 items-start justify-end gap-2 lg:items-center lg:justify-start lg:gap-3">
                <span className="w-9 h-9 rounded-xl flex items-center justify-center shrink-0 lg:w-8 lg:h-8 lg:rounded-lg" style={{ background: 'rgba(0, 199, 177, 0.1)', border: '1px solid rgba(0, 199, 177, 0.2)' }}>
