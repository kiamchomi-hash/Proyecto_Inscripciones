...
- `components/ui/` es la biblioteca de piezas visuales reutilizables (botones, tarjetas, interruptores), cada una con su CSS al lado e importado desde su propio `.tsx`, como `beneficios-strip`. `/laboratorio` las muestra renderizadas y **sólo existe en desarrollo**: en producción devuelve 404, así que no está en el sitemap ni en `lib/vigilancia-esperado.ts`. Las reglas de qué hay que arreglarle a una pieza traída de afuera —clases prefijadas, colores por variable, el `rem` que acá no mide 16px, la CSP que bloquea las fuentes de Google— están en `components/ui/LEER.md`.
