...
- **CTR por debajo de lo esperado para su posición**: la página ya rankea y la gente no entra. Es título y descripción, no contenido. Es el hallazgo con mejor relación esfuerzo/resultado del informe: no hay que esperar a que Google reevalúe nada, el cambio se ve en días.
- **Consultas cerca de los primeros lugares** (posición 4–15): lo único que se mueve con trabajo de página. Más arriba ya se ganó; más abajo hace falta otra cosa (enlaces, tiempo) y no la vas a resolver esta semana. Fijate si la consulta y la página que la recibe se corresponden: cuando una consulta cae en una página que no es la que debería, el problema es de enlace interno o de canibalización, no de texto.
...
- **Enlaces internos entre fichas**: la página de carrera ya enlaza 6 del mismo nivel más 2 de otro, con un corte que rota según el `id`. Es determinístico a propósito, para no romper el caché de ISR — si proponés cambiarlo, decí cómo se mantiene esa propiedad.
