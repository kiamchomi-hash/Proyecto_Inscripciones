...
2. **Dashboard Comercial de Teclab** — `https://informacion.teclab.edu.ar/hubfs/ADMISION/CALIDAD%20Y%20%20TRAINING/Dashboard_Comercial_Teclab%20(Agentes).html`, el panel que Teclab arma para sus asesores de admisión. Desde el 26/08/2026 reemplaza al archivo `(01).html`. Es el equivalente de La Nube para Teclab y manda sobre el sitio comercial. **El HTML es público y el login es de JavaScript**: los datos viajan embebidos, así que se baja con `curl` y se extraen las constantes del script (`DATA`, `BENCHX`, `GUIAS`, `CONV`) balanceando llaves y evaluándolas en un `vm`. El acceso visual usa las credenciales entregadas por Teclab, que no se versionan. El snapshot local todavía corresponde a la fuente anterior; su actualización está anotada en `PENDIENTES.md`.
...
7. **Documentos internos** (`ventas/fuentes/teclab/faq_ventas_y_modalidad.md`, guías de WhatsApp). Sirven, pero no alcanzan para afirmarle algo a un lead si una fuente oficial dice otra cosa.
...
- **La financiación de Teclab la manda la placa «Opciones de financiación» del período 2B**, vigente desde el 26/08/2026. Visa y Mastercard: 3 cuotas al 6,15% de interés y 43,34% CFT con Santander, Galicia, BBVA, Nación, Macro y Naranja; para el resto, 7,41% y 53,95%. No ofrece 6 cuotas bancarias. Naranja X tiene 1 pago y Plan Z en 3 cuotas sin interés; GoCuotas y WiBond, hasta 6 sin interés. AMEX emitida por Santander o directamente por American Express admite 1 o 3 cuotas; el resto, sólo 1 pago. El simulador del 27/08 contradijo esa placa y queda sólo como diagnóstico: no puede sobrescribir `carreras/teclab/financiacion-teclab-oficial.json`.
...
**Identidad Argentina sigue siendo el trabajo pendiente**: es la única de las tres sin una fuente oficial procesada. Su sitio bloquea el scraping (robots + SPA); el material de trabajo queda en `carreras/identidad/fuentes/` dentro del proyecto y no se versiona.
...
- **Financiación mensual.** No existe pago mensual: el reglamento compromete el pago por cuatrimestre o bimestre. Las cuotas son financiación del medio de pago según la placa vigente. Las consultas de "¿cuánto por mes?" siguen contestándose con el total.
