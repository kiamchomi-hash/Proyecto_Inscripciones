...
- [ ] **Sacar los secretos de webhook del cuerpo de los triggers y llevarlos al Vault de Supabase.** `notify_revalidar` y `notify_edge_function` llevan el token escrito como literal dentro del `jsonb_build_object` del header (`'Authorization', 'Bearer <secreto>'`). `pg_proc.prosrc` lo lee cualquier rol que pueda conectarse a la base, así que `REVALIDATE_SECRET` (32 caracteres) y `WEBHOOK_SECRET` (64) quedan a la vista de cualquier credencial de conexión, incluida `cau_editor`, que fue creada justamente para no tener alcance de más. Detectado el 28/08/2026 al buscar `REVALIDATE_SECRET` para `.env.local`.
  ...
  **Al escribirlo aparecieron dos cosas que la entrada original no tenía.** Primero, de `WEBHOOK_SECRET` hay **tres** copias en la base, no dos: los dos triggers y el `command` del job `digest-clicks-diario`, que se lo comió pegado del `format()` de `sql/2026-07-27_cron_digest_clicks.sql`. Esa tercera no está expuesta a `cau_editor` (`cron` le da `permission denied`), pero es el mismo valor y hay que rotarla igual. Segundo, `WEBHOOK_SECRET` lo validan **dos** Edge Functions, `notificar` y `digest-clicks`; `alerta-firewall` no, que usa `VERCEL_WEBHOOK_SECRET`, otro secreto que nunca estuvo en la base.

  Queda partido en dos archivos, y el orden entre ellos importa:

  - ~~`sql/2026-08-28_secretos_al_vault.sql` — **Fase 1, sin rotar**.~~ **Aplicada el 28/08/2026.** Lee el valor vigente del literal que todavía estaba en `pg_proc`, lo mete al Vault y reescribe los tres lugares para que lo lean de ahí. Como el valor no cambia, no hubo ventana de 401 en ningún lado. `notify_edge_function` pasó a `SECURITY DEFINER` (venía sin serlo, y el rol que hace el `INSERT` no llega al Vault), con su `REVOKE EXECUTE` al lado. Verificado: `pg_proc` da `tiene_literal = false` en las dos funciones desde `cau_editor`, y los tres caminos responden 200 — revalidación (`{"ok":true,"rutas":[...]}`), `notificar` (`{"ok":true,"telegram":true}`) y `digest-clicks` (`{"ok":true,"fecha":"2026-08-27","carreras":5,"telegram":true}`). O sea que `SECURITY DEFINER` sí alcanza `vault.decrypted_secrets`, que era el único riesgo del diseño.
  - `sql/2026-08-28_rotar_secretos.sql` — **Fase 2, la rotación**. Con la Fase 1 hecha la base tiene el secreto en un solo lugar, así que rotar es un `UPDATE` de una línea por secreto. Los valores nuevos los genera `node herramientas/generar-secretos.mjs`, que no los escribe en ningún archivo (el repo es público) y deja el SQL en el portapapeles.

  La ventana de la Fase 2 no se puede eliminar, sólo achicar, y cae distinto en cada uno: en `REVALIDATE_SECRET` deja una página vieja hasta que venza su `revalidate` (se arregla solo), y en `WEBHOOK_SECRET` deja un formulario guardado **sin aviso de Telegram** — el lead no se pierde, pero nadie se entera hasta que se mire la tabla. Por eso Vercel va primero (necesita redeploy, que es el paso lento) y el `UPDATE` del Vault va inmediatamente después.

  Como los dos secretos son independientes, la Fase 2 se partió también: **`REVALIDATE_SECRET` rotado el 28/08/2026** (el que no tiene ventana peligrosa), y `WEBHOOK_SECRET` queda para un rato de poco tráfico. El valor nuevo se puso con `npx vercel env rm` + `env add` leyendo de stdin, así que nunca se imprimió en pantalla ni quedó en un archivo; después va `npx vercel redeploy <url del último prod>`, que rehace el mismo commit con la variable nueva — **no `vercel --prod`, que buildea desde el working tree y subiría lo que esté sin commitear**. Verificado con un 200 y `{"ok":true,"rutas":[...]}`.

  **Falta rotar `WEBHOOK_SECRET`.** Ahí el paso equivalente es `npx supabase secrets set`, que no necesita redeploy, así que la ventana es más corta pero más cara: hay que correr el bloque 2 de `sql/2026-08-28_rotar_secretos.sql` enseguida y verificar los **dos** consumidores, `notificar` y `digest-clicks`.
