...
- [ ] **Rotar `TURNSTILE_SECRET_KEY` y marcarla Sensitive.** Vercel avisa que cualquiera con acceso al proyecto puede leer su valor, y al 28/07 `vercel env pull --environment=production` la sigue devolviendo en claro. No hubo filtración: la clave sólo se usa server-side (`lib/turnstile.ts` tiene `import 'server-only'`) y nunca entra al bundle del navegador.
...
**El captcha no se puede probar automatizado.** Cloudflare no emite token para un navegador manejado por Playwright, ni headless ni con ventana visible. El chequeo rápido del vencimiento es mirar el **desmarque**: pasados los 300 s el checkbox se vacía solo y el botón se apaga; no hace falta llegar a enviar. Y al medir layout en producción hay que esperar a que la altura se estabilice, porque el widget monta un iframe de ~70 px después del `load` (en local no se renderiza: falta `NEXT_PUBLIC_TURNSTILE_SITE_KEY`).
