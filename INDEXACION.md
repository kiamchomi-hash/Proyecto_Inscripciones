...
**Medición: 07/08/2026.** Sitemap: 113 URLs, 91 carreras.
**87 de 91 carreras (96%)** · fuera de `/carreras/`: 20 de 22.

Antes: 82 el 05/08, 82 el 04/08, 75 el 03/08, 76 el 01/08, 56 el 29/07, 23 el 24/07.
...
Último rastreo 11/06: Google sirve un calendario vencido de hace dos meses.

Si sobra cuota: Lic. en Informática (04/06), Lic. en Administración (26/06) y Actuario (27/06). Rinden poco.
...
Las 91 del sitemap. La fecha es el último rastreo; ❌ = `last_crawled: Never`.
...
### Fuera de `/carreras/` (20 / 22)

- ✅ Home (06/08), `/faq` (19/07), `/contacto` (19/07), `/sobre-nosotros` (21/07)
- ✅ Clases de apoyo: `/clases-apoyo` (11/06 — el recrawl), `arte` (06/08), `computacion` (18/07), `lengua` (06/08), `matematica` (06/08)
- ✅ Novedades: `/novedades/1` (19/07) y los 10 artículos
- ❌ `/clases-apoyo/fisico-quimica` — *(desconocida)*, entró al sitemap después de la medición
- ❌ `/novedades/2` — a propósito

`ingles` y `secundario` están fuera del sitemap con `noindex` mientras sigan `en_construccion`.

---

## Cómo volver a medir

Sacar las URLs del sitemap —**no reusar la lista de acá**— y pasarlas por `batch_url_inspection` de a 10 (con más falla); se pueden lanzar lotes en paralelo, ~2 min cada uno. Contar `coverage_state: "Submitted and indexed"`.

`Discovered - currently not indexed` (está en la cola) es distinto de `URL is unknown to Google` (ni la descubrió). Ese campo oscila entre mediciones: no leerlo como regresión. El informe "Indexación → Páginas" del panel atrasa días; la verdad es la inspección de URL.
